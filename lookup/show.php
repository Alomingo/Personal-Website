...
<body>
  <div align="center">
    ...
    <a class=back href="../users/index.php">首页</a>
  <?php
    require_once  $_SERVER["DOCUMENT_ROOT"] . './inc/db.php';
    require_once  $_SERVER["DOCUMENT_ROOT"] . './inc/common.php';
    
    $id=$_GET['id'];
    $querygame=$db->prepare('select * from games where id=:id');//游戏名
    $querygame->bindValue(':id',$id,PDO::PARAM_STR);
    $querygame->execute();
    $postgame=$querygame->fetchObject();

      $querycat=$db->prepare('select name from catalog where id =:id');//当前分类
      $querycat->bindValue(':id',$postgame->catalog,PDO::PARAM_STR);
      $querycat->execute();
      $postcatalog=$querycat->fetchObject();

    $querytagbk=$db->prepare('select * from tags_games where game_id =:id');//属性
    $querytagbk->bindValue(':id',$id,PDO::PARAM_STR);
    $querytagbk->execute();
    $posttaggame=$querytagbk->fetchObject();
    $tagid=$posttaggame->tag_id;
